(() => {
  ...
  function render() {
    ...
    body.innerHTML = visible.length ? visible.map((quiz) => { const status = statusOf(quiz); const title = valueOf(quiz, ["title", "name"], "Untitled quiz"); const topic = valueOf(quiz, ["topic", "chapter"], "No topic"); const questions = valueOf(quiz, ["question_count"], quiz.questions?.length || 0); const responses = valueOf(quiz, ["response_count", "responses"], 0); const score = Number(valueOf(quiz, ["average_score", "avg_score"], 0)) || 0; const classValue = valueOf(quiz, ["class_grade", "grade", "class"], "-"); const classLabel = /^class\s/i.test(String(classValue)) ? classValue : `Class ${classValue}`; const id = escapeHtml(quiz.id); return `<tr data-quiz-id="${id}"><td><div class="item-title"><div class="item-icon quiz-row-icon"><i class="fa-solid fa-clipboard-question"></i></div><div><div class="quiz-title-text">${escapeHtml(title)}</div><div class="quiz-subtitle-text">${escapeHtml(topic)}</div></div></div></td><td>${escapeHtml(classLabel)}</td><td>${escapeHtml(valueOf(quiz, ["subject"], "-"))}</td><td>${escapeHtml(questions)}</td><td>${escapeHtml(responses)}</td><td><div class="progress-bar-container"><span>${score}%</span><div class="progress-bar"><div class="progress-fill" style="width:${Math.min(100, score)}%"></div></div></div></td><td><span class="badge-status status-${escapeHtml(status)}">${escapeHtml(status[0].toUpperCase() + status.slice(1))}</span></td><td>${formatDate(valueOf(quiz, ["created_at", "updated_at"]))}</td><td><div class="action-icons"><a href="create_quiz.html?id=${id}" title="Edit"><i class="fa-solid fa-pen"></i></a><button data-delete="${id}" title="Delete"><i class="fa-solid fa-trash-can"></i></button></div></td></tr>`; }).join("") : '<tr><td class="quiz-empty-state" colspan="9"><strong>No quizzes found</strong>Create a quiz or adjust your search and filters.</td></tr>';
